...
export default function TelaoLobbyScreen({
  roomCode,
  players,
  onStartGame,
}: TelaoLobbyScreenProps) {
  ...
  useEffect(() => {
    ...
    if (phase >= 2 && phase < 5) {
      spamInterval = setInterval(() => {
        setFakePlayerCount(
          (prev) => prev + Math.floor(Math.random() * 47) + 13,
        );
      }, 50);
    }
  ...
  return (
    <div className="flex-1 flex flex-col h-screen max-h-screen bg-[#1C1C1C] overflow-hidden selection:bg-[#FF6B35] selection:text-[#F7F5F0]">
      ...
      >
        ...
        <div
          className={`flex-1 border-[4px] border-[#1C1C1C] rounded-[8px] flex flex-col overflow-hidden relative shadow-[8px_8px_0px_#1C1C1C] transition-colors duration-300 ${phase >= 4 ? "bg-[#C8381E] shadow-none" : "bg-[#EDEBE5]"}`}
        >
          <div
          ...
          <div className="bg-[#1C1C1C] text-[#F7F5F0] p-2 md:p-3 flex justify-between items-center border-b-[4px] border-[#1C1C1C] z-10 transition-all duration-[600ms]">
            <div className="flex items-center gap-4 font-pixel text-base md:text-xl tracking-widest">
              <span
                className={`flex items-center gap-2 ${phase >= 2 ? "text-[#C8381E] animate-pulse" : "text-[#FF6B35]"}`}
              >
                <div
                  className={`w-2 h-2 md:w-3 md:h-3 rounded-full ${phase >= 2 ? "bg-[#C8381E] animate-none" : "bg-[#FF6B35] animate-ping"}`}
                ></div>
                {phase >= 2 ? "ANOMALY_DETECTED" : "NETWORK_LOBBY_ACTIVE"}
              </span>
              <span className="hidden md:inline border-l-2 border-[#333] pl-4 text-[#888888]">
                {phase >= 2
                  ? "MULTIPLE_REQUESTS_SPIKE!!!"
                  : `AWAITING_NODES${dots}`}
              </span>
            </div>
            <div
          ...
          <div className="flex-1 flex flex-col lg:flex-row overflow-visible z-10 relative">
            ...
            >
              <div>
                <div
                  className={`inline-block border-[4px] border-[#1C1C1C] px-3 py-1 font-pixel text-lg md:text-xl tracking-widest uppercase mb-4 ${phase >= 4 ? "bg-[#F7F5F0] text-[#C8381E] shadow-none" : "bg-[#1C1C1C] text-[#F7F5F0] shadow-[4px_4px_0px_#FF6B35]"}`}
                >
                  {"// ACCESS_PROTOCOL"}
                </div>

                <h2
                  className={`font-pixel text-[clamp(1.8rem,4vw,2.5rem)] uppercase leading-none mb-4 border-l-[6px] pl-4 tracking-widest ${phase >= 4 ? "border-[#1C1C1C] text-[#F7F5F0]" : "border-[#FF6B35] text-[#1C1C1C]"}`}
                >
                  {phase >= 4
                    ? "SISTEMA_COMPROMETIDO_ FALHA_GERAL"
                    : "Acesse o terminal mobile para ingressar na sessão."}
                </h2>

                <p
                  className={`font-pixel text-[clamp(1.2rem,2vw,1.8rem)] mb-6 leading-tight ${phase >= 4 ? "text-[#1C1C1C]" : "text-[#888]"}`}
                >
                  {phase >= 4 ? (
                    "A TENTATIVA DE CONEXÃO CAUSOU UMA SOBRECARGA CRÍTICA. ACESSO NEGADO."
                  ) : (
                    <>
                      Acesse{" "}
              ...
              <div className="flex flex-col items-center my-auto">
                <div
                  className={`w-full border-[4px] border-[#1C1C1C] p-4 md:p-6 rounded-[8px] relative overflow-hidden flex flex-col items-center justify-center ${phase >= 4 ? "bg-[#C8381E] shadow-none animate-pulse" : "bg-[#1C1C1C] shadow-[8px_8px_0px_#FF6B35]"}`}
                >
                  <div
                    className="absolute inset-0 pointer-events-none opacity-20"
                    style={{
                      backgroundImage:
                        "repeating-linear-gradient(0deg, transparent, transparent 4px, #000 4px, #000 8px)",
                    }}
                  ></div>

                  <span
                    className={`font-pixel text-lg md:text-xl tracking-widest mb-1 z-10 ${phase >= 4 ? "text-[#1C1C1C] font-bold" : "text-[#888888]"}`}
                  >
                    SESSION_HASH:
                  </span>

                  <span
                    className={`font-pixel text-[clamp(2.5rem,7vw,6rem)] leading-none tracking-widest z-10 w-full text-center whitespace-nowrap ${phase >= 4 ? "text-[#1C1C1C]" : "text-[#F7F5F0]"}`}
                    style={{
                      textShadow: phase >= 4 ? "none" : "4px 4px 0px #FF6B35",
                    }}
                  >
            ...
            >
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex gap-1 h-3 items-end">
                    {[...Array(10)].map((_, i) => (
                      <div
                        key={i}
                        className={`w-2 ${phase >= 3 ? "bg-[#C8381E] h-full animate-pulse" : i < 2 ? "bg-[#FF6B35] h-full" : "bg-[#D0CEC8] h-1"}`}
                      ></div>
                    ))}
                  </div>
                </div>
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex gap-1 h-3 items-end">
                    {[...Array(10)].map((_, i) => (
                      <div
                        key={i}
                        className={`w-2 ${phase >= 3 ? "bg-[#C8381E] h-full animate-pulse" : i < 4 ? "bg-[#1C1C1C] h-full" : "bg-[#D0CEC8] h-1"}`}
                      ></div>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex justify-between items-end mb-6 border-b-[4px] border-dashed border-[#1C1C1C] pb-2 z-10">
                <h3 className="font-pixel text-[clamp(1.5rem,3vw,2.5rem)] tracking-widest text-[#1C1C1C] leading-none">
                  CONNECTED_NODES
                </h3>
                <span
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-5 z-10">
                {players.map((player) => (
                  <div
                    key={player.id}
                    className={`bg-[#F7F5F0] border-[4px] border-[#1C1C1C] border-b-[6px] rounded-[4px] p-3 flex flex-col items-center justify-center relative overflow-hidden transition-all ${phase >= 3 ? "border-[#C8381E] shadow-none bg-[#C8381E]" : "shadow-[4px_4px_0px_#1C1C1C]"}`}
                  >
                    <div
                      className={`absolute top-0 left-0 w-full h-1 ${phase >= 3 ? "bg-[#1C1C1C]" : "bg-[#FF6B35]"}`}
                    ></div>

                    <div
                      className={`text-4xl md:text-5xl mb-2 mt-2 font-pixel ${phase >= 3 ? "text-[#F7F5F0] animate-ping" : "text-[#1C1C1C]"}`}
                    >
                      {phase >= 3 ? "(!)" : player.avatar}
                    </div>

                    <div
                      className={`w-full text-center py-1 font-pixel text-base md:text-lg tracking-widest truncate px-2 ${phase >= 3 ? "bg-[#F7F5F0] text-[#C8381E]" : "bg-[#1C1C1C] text-[#F7F5F0]"}`}
                    >
                      {phase >= 3 ? "UNKNOWN_NODE" : player.name}
                    </div>

                    <div
                      className={`mt-2 text-xs font-pixel tracking-widest ${phase >= 3 ? "text-[#1C1C1C] font-bold" : "text-[#888888]"}`}
                    >
                      {phase >= 3
                        ? `ERR_0x${Math.floor(Math.random() * 9999)}`
                ...
                {Array.from({ length: emptySlotsCount }).map((_, i) => (
                  <div
                    key={`empty-${i}`}
                    className={`border-[4px] border-dashed rounded-[4px] p-3 flex flex-col items-center justify-center ${phase >= 3 ? "border-[#C8381E] bg-[#C8381E] opacity-100" : "border-[#D0CEC8] bg-transparent opacity-60"}`}
                  >
                    <div
                      className={`text-4xl mb-2 font-pixel ${phase >= 3 ? "text-[#F7F5F0]" : "text-[#D0CEC8]"}`}
                    >
                      {phase >= 3 ? "(!)" : "(?)"}
                    </div>
                    <div
                      className={`w-full text-center py-1 font-pixel text-base md:text-lg tracking-widest truncate px-1 ${phase >= 3 ? "bg-[#F7F5F0] text-[#C8381E]" : "bg-[#D0CEC8] text-[#EDEBE5]"}`}
                    >
                      {phase >= 3 ? "BREACH" : "EMPTY_SLOT"}
                    </div>
                    <div
                      className={`mt-2 text-xs font-pixel tracking-widest ${phase >= 3 ? "text-[#1C1C1C] font-bold" : "text-[#888888] opacity-0"}`}
                    >
